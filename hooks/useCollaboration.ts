 ...
 * Manages real-time collaboration state and syncing with Supabase
 *
 * Migration Note: Now uses Supabase Realtime instead of Firestore.
 * Firebase is still available for auth-related collaboration features.
 */
...
export function useCollaboration({
  ...
  useEffect(() => {
    ...
    if (sharedId && isSupabaseAvailable) {
  ...
  }, [isSupabaseAvailable, scriptData, moodBoards, globalHistory]);
  ...
  }, [isSupabaseAvailable, setScriptData, setMoodBoards, setGlobalHistory]);
  ...
  return {
    ...
    isFirebaseAvailable: isSupabaseAvailable
  };
}
