...
import { X, Loader2, CheckCircle2, AlertCircle, Image as ImageIcon, Download, ImagePlus, Check } from 'lucide-react';
...
export const CoverageGeneratorModal: React.FC<CoverageGeneratorModalProps> = ({
  ...
  // Set default pack based on entity type
  const getDefaultPackId = () => {
    if (entityType === 'location') return 'location';
    if (entityType === 'product') return 'productHero';
    return 'contactSheet'; // character default
  };
  const [selectedPackId, setSelectedPackId] = useState<string>(getDefaultPackId());
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-4 p-4 bg-zinc-800/50 rounded-xl">
                ...
                <div className="ml-auto flex gap-2">
                  ...
                  <button
                    onClick={() => {
                      if (library && onComplete && !refsAdded) {
                        onComplete(library);
                        setRefsAdded(true);
                      }
                    }}
                    disabled={refsAdded || library.generatedAngles === 0}
                    className={`px-4 py-2 ${refsAdded ? 'bg-blue-800' : 'bg-blue-600 hover:bg-blue-700'} disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2`}
                  >
                    {refsAdded ? (
                      <><Check className="w-4 h-4" /> Added to {entityName}</>
                    ) : (
                      <><ImagePlus className="w-4 h-4" /> Add to References</>
                    )}
                  </button>
                  <button
