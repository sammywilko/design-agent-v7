...
// Batch generation with graceful error handling, retries, and progress tracking
 ...
 *
 * Features:
 * - Parallel generation within batches (default 10 at a time)
...
export async function generateBatch(
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    ...
    if (isCancelled?.()) {
      ...
      // Mark remaining prompts as cancelled
      const remainingPrompts = batches.slice(batchIndex).flat();
    ...
    const batchResults = await Promise.all(
      batch.map(async (prompt): Promise<BatchResult> => {
        // Check cancellation before each individual generation
        if (isCancelled?.()) {
          return {
            prompt,
            imageUrl: '',
    ...
    // Rate limiting: Small delay between batches to avoid rate limits
  ...
  const totalCancelled = allResults.filter(r => r.status === 'cancelled').length;
  ...
  if (totalCancelled > 0) {
    console.log(`   Cancelled: ${totalCancelled}/${total}`);
  }
...
export function getBatchStats(results: BatchResult[]) {
  ...
  const totalTime = successfulResults.reduce((sum, r) => sum + (r.generationTime || 0), 0);
...
/**
 * Create a rate-limited version of a generate function
 * Useful when you need to throttle API calls
 */
export function createRateLimitedGenerator(
  generateFn: (prompt: string) => Promise<string>,
